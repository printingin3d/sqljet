...
public class SqlJetIndexedMemPages {
    ...
    private static void balanceNonroot(SqlJetMemPage pParent, int iParentIdx, ISqlJetMemoryPointer aOvflSpace,
        ...
        try {
            ...
            /* u16 */ int[] szCell = new int[nMaxCells]; /*
                                                          * Local size of all
                                                          * cells in apCell[]
                                                          */
            ...
            int pageFlags = apOld[0].getData()
                    .getByteUnsigned(0); /* Value of pPage->aData[0] */
            for (i = 0; i < k; i++) {
                SqlJetMemPage pNew;
                if (i < nOld) {
     ...
     *
     * Instead of trying to balance the 3 right-most leaf pages, just add a new
     ...
     *
     * The pSpace buffer is used to store a temporary copy of the divider cell
     * that will be inserted into pParent. Such a cell consists of a 4 byte page
     * number followed by a variable length integer. In other words, at most 13
     * bytes. Hence the pSpace buffer must be at least 13 bytes in size.
     */
     ...
     *
     * A new child page is allocated and the contents of the current root page,
     * including overflow cells, are copied into the child. The root page is
     * then overwritten to make it an empty page with the right-child pointer
     * pointing to the new page.
     *
     * Before returning, all pointer-map entries corresponding to pages that the
     * new child-page now contains pointers to are updated. The entry
     * corresponding to the new right-child pointer of the root page is also
     * updated.
     *
     * If successful, *ppChild is set to contain a reference to the child page
     * and SQLITE_OK is returned. In this case the caller is required to call
     * releasePage() on *ppChild exactly once. If an error occurs, an error code
     * is returned and *ppChild is set to 0.
     */
