...
public abstract class SqlJetEngine {
    ...
    private static final Set<SqlJetFileOpenPermission> READ_PERMISSIONS = Collections
            .unmodifiableSet(SqlJetUtility.of(SqlJetFileOpenPermission.READONLY));
    private static final Set<SqlJetBtreeFlags> WRITE_FLAGS = Collections
            .unmodifiableSet(SqlJetUtility.of(SqlJetBtreeFlags.READWRITE, SqlJetBtreeFlags.CREATE));
    ...
    public SqlJetEngine(final File file, final boolean writable, final String fsName) throws SqlJetException {
        this(file, writable, assertNotNull(SqlJetFileSystemsManager.getManager().find(fsName), SqlJetErrorCode.MISUSE,
                String.format("File system '%s' not found", fsName)));
    }
    ...
    protected void refreshSchema() throws SqlJetException {
        if (null == btree.getSchema() || !getOptions().verifySchemaVersion()) {
            readSchema();
        }
    }
     ...
     * @throws SqlJetException
     */
    public <T> T runEngineTransaction(final ISqlJetTransaction<T, SqlJetEngine> op, @Nonnull SqlJetTransactionMode mode)
            throws SqlJetException {
        checkOpen();
    ...
    private boolean isTransactionStarted(final SqlJetTransactionMode mode) {
        return transactionMode != null && (transactionMode == mode || mode == SqlJetTransactionMode.READ_ONLY);
    }
