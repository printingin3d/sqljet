...
public class RowCountExampleTest {
	...
	public void setUp() throws SqlJetException {
		...
		try {
			...
			classLogger.log(Level.CONFIG, "2 tables created in a new data file {0}.",
					new Object[] { SqlJetDb.IN_MEMORY, persistenceDb.getSchema().getTableNames() });
			...
			Long time = Long.valueOf(update.getTime());
			groupTable.insert(ONE, time, "ACTIVE", ZERO, Boolean.TRUE);
			groupTable.insert(TWO, time, "INACTIVE", ONE, Boolean.FALSE);
			customerTable.insert(ONE, ONE, time, "SERVING", "nobody@example.com", "ENCOM1", ZERO, Boolean.TRUE);
			customerTable.insert(TWO, ONE, time, "SERVING", "nobody@example.com", "ENCOM2", ONE, Boolean.TRUE);
			customerTable.insert(THREE, ONE, time, "SERVING", "nobody@example.com", "ENCOM3", THREE, Boolean.TRUE);
			customerTable.insert(FOUR, ONE, time, "SERVING", "nobody@example.com", "ENCOM4", TWO, Boolean.TRUE);
		} finally {
	...
	public void testRowCount() throws SqlJetException {
		// open all the tables here so we aren't opening tables repeatedly in
		// the loops.
		persistenceDb.getTable(GROUP_TABLE_NAME);
		ISqlJetTable customerTable = persistenceDb.getTable(CUSTOMER_TABLE_NAME);
		// begin a read transaction before reading data (this is required by
		// SqlJet).
		persistenceDb.beginTransaction(SqlJetTransactionMode.READ_ONLY);
		try {
			classLogger.log(Level.CONFIG, "Getting Group ID {0} to merge Customer ID {1}.", new Object[] { ONE, ONE });
			ISqlJetCursor foundRow = customerTable.lookup(customerTable.getPrimaryKeyIndexName(), ONE);
			if (foundRow != null && foundRow.first()) {
				int rowCount = intFromLong(foundRow.getRowCount());
				classLogger.log(Level.CONFIG, "Reading {0} Customers.", Integer.valueOf(rowCount));
			}
		} finally {
			persistenceDb.commit();
		}
	 ...
	 * objects or calling complex functions. It's horribly inaccurate once the
	 * value starts to "wrap", but it avoids issues associated with a straight
	 * cast, and it's almost as fast.
	 * 
	 * @param input
	 *            a long that is expected to be between 0 and Integer.MAX_VALUE.
